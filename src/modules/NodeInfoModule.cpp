...
meshtastic_MeshPacket *NodeInfoModule::allocReply()
{
    ...
    // If we sent our NodeInfo less than 15 min. ago, don't send it again as it may be still underway. (Was 5 minutes in official firmware)
    if (!shorterTimeout && lastSentToMesh && Throttle::isWithinTimespanMs(lastSentToMesh, 15 * 60 * 1000)) {
        LOG_DEBUG("Skip send NodeInfo since we sent it <15min ago");
        ignoreRequest = true; // Mark it as ignored for MeshModule
        return NULL;
